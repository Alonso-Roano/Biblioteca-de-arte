  ...
  >
    <div class="flex flex-col gap-4">
      ...
      <div>
        ...
        <InputText v-model="descripcionObra" class="w-full" :class="{ 'p-invalid': errors.descripcionObra }" />
        <small class="p-error" v-if="errors.descripcionObra">{{ errors.descripcionObra }}</small>
      </div>
      <div>
...
const schema = yup.object({
  ...
  descripcionObra: yup.string().required('La descripción es requerida'),
...
const { value: descripcionObra } = useField('descripcionObra');
...
watch(
  ...
  (newObra) => {
    ...
    nombre.value = newObra.nombre || '';
    descripcionObra.value = newObra.descripcionObra || '';
    nombreCorto.value = newObra.nombreCorto || '';
  },
  { immediate: true }
);

const submitForm = handleSubmit((values) => {
  ObraEdit.value = { ...values, id: idObra.value };
