<template>
    <div class="m-2">
      <div class="flex w-full justify-between mb-1.5">
        <h2 class="block text-3xl text-black p-2 font-semibold">Comentarios - {{ totalRecords }} en total</h2>
        <div @click="$emit('add-Comentario')"
      ...
      <DataTable :value="ComentariosShow" tableStyle="min-width: 50rem" @page="onPageChange">
        ...
        <Column header="Acciones">
          <template #body="props">
            <span class="block">
              <Button icon="pi pi-eye" variant="text" rounded aria-label="Ver" @click="$emit('view-Comentario', props.data)">
...
<script setup lang="ts">
  ...
  watch(Comentarios, (newComentarios) => {
    const Usuarios = <any> newComentarios;
    ComentariosShow.value = Usuarios.items;
    totalRecords.value = Usuarios.total;
  }, { deep: true });

  watch([filters, page, limit], loadComentarios, { immediate: true });
