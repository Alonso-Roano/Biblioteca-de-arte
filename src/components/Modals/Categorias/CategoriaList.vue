<template>
    <div class="m-2">
      <div class="flex w-full justify-between mb-1.5">
        <h2 class="block text-3xl text-black p-2 font-semibold">Categorias - {{ totalRecords }} en total</h2>
        <div @click="$emit('add-Categoria')"
      ...
      <DataTable :value="CategoriasShow" tableStyle="min-width: 50rem" @page="onPageChange">
        ...
        <Column header="Acciones">
          <template #body="props">
            <span class="block">
              <Button icon="pi pi-eye" variant="text" rounded aria-label="Ver" @click="$emit('view-Categoria', props.data)">
...
<script setup lang="ts">
  ...
  watch(Categorias, (newCategorias) => {
    const Usuarios = <any> newCategorias;
    CategoriasShow.value = Usuarios.items;
    totalRecords.value = Usuarios.total;
  }, { deep: true });

  watch([filters, page, limit], loadCategorias, { immediate: true });
