...
<template>
  <div class="bg-[#EDE7DD] min-h-screen">
    ...
    <div class="flex flex-col items-center py-10 px-6 relative">
      <!-- Sección de imágenes en 2 columnas en pantallas medianas y más -->
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 z-10">
        <div v-for="(image, index) in img.slice(0, 2)" :key="index"
          class="relative group overflow-hidden rounded-xl p-4">
      ...
      <!-- Imagen centrada en pantallas grandes -->
      <div class="grid grid-cols-1 gap-6 mt-12 z-10">
        <div v-for="(image, index) in img.slice(2, 3)" :key="index"
          class="relative group overflow-hidden rounded-xl sm:ml-[-10rem] md:ml-[-28rem]">
          <img :src="image.src" :alt="image.alt"
            class="w-full max-w-[80%] h-auto object-cover transition-transform transform group-hover:scale-105">
          <div
            class="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
            <span class="text-white text-lg font-semibold">{{ image.alt }}</span>
          </div>
      ...
      <!-- Sección de imágenes en 2 columnas -->
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-12 z-10">
        <div v-for="(image, index) in img.slice(3, 5)" :key="index"
          class="relative group overflow-hidden rounded-xl p-4">
          <img :src="image.src" :alt="image.alt"
            class="w-full max-w-[90%] h-auto object-cover transition-transform transform group-hover:scale-105">
          <div
      ...
      <!-- Sección final con 3 columnas en pantallas grandes -->
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-12 z-10">
        <div v-for="(image, index) in img.slice(5)" :key="index"
          class="relative group overflow-hidden rounded-xl p-4">
          <img :src="image.src" :alt="image.alt"
            class="w-full max-w-[90%] h-auto object-cover transition-transform transform group-hover:scale-105">
          <div
      ...
      <!-- Líneas de fondo (z-0 para que estén detrás de las imágenes) -->
