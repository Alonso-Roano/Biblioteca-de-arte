...
const cargarLibros = async () => {
  try {
    ...
    const nuevosLibros = await apiRequest("libro.obtenerPorAutor", { token: tokenEnviar });
...
<template>
  <div class="min-h-screen bg-[#fefcff]">
    <h1 class="text-left m-5 text-3xl font-bold text-[#5d20d3]">Libros que creaste</h1>
    <a href="/libro/crear">Agregar</a>
    ...
    <div v-if="libros.length > 0" class="grid grid-cols-1 place-items-center sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4">
      <div v-for="libro in libros" :key="libro.titulo" class="mb-10">
        <a class="book-container" :href="`/Libro/Editar/${store.user?.id}/${libro.slug}`"  rel="noreferrer noopener">
